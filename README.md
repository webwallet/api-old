...
### Definitions
  ...
  - **Wallet Address**  
  An identifier that represents a payment credential or a possible destination of a transaction. Wallet addresses are to be found in the {walletAddress} segment of WebWallet URLs, and are generated by base58check-encoding a prefixed 20-bytes hash of a public key:

  ```
  wUhBhfiaDxpYwdE754TU8XH49ayDjqM3bR      [base58check('87' + hash20bytes(publicKey))]
  ```
  - **Transaction**  
  An operation that transfers currency units between addresses by atomically modifying their balances in a proportional way, so that the amount added to the recipient addresses equals the amount subtracted from the sender addresses.
  ...
  - **Transaction Output**  
  A message that specifies an aggregate change of an address balance after processing one or more transaction inputs. It is a statement about the past that fulfills the promises made by those inputs. This message is a reference to a document that reflects the resulting balance of an address after processing a transaction.
