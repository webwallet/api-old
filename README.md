...
### Definitions
  ...
  ```
  wUhBhfiaDxpYwdE754TU8XH49ayDjqM3bR      [base58check(hash20bytes(publicKey))]
  ```
  
  - **Transaction Request**  
  A message that specifies the source, destination, amount and currency of a transaction, along with the proof of consent of the sending party to participate in the transaction.

  - **Transaction Record**  
  A document that gathers all information required for a transaction request to be processed and included in a transaction chain, such as the proofs of consent of all involved parties.
